...
export default function Card({ name, price, stock,sold, image }: { name: string; price: number; stock: number; category: string; sold: number; image: string }) {
    return (
        <div className="bg-white shadow-md rounded-lg p-4 flex flex-col  gap-2 w-40 md:w-60">
            
            <img src={image} alt={name} className=" object-contain aspect-[4/3] w-30 h-30 md:h-40 md:w-60" />
